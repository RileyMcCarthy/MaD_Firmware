...
i2cEncoderLibV2::i2cEncoderLibV2(uint8_t add)
{
	...
	readMask = 0b00000001;
	writeMask = 0b11111110;
	_add = (add << 1); //converts the address to form allowing I2C masks set write/read
}

void i2cEncoderLibV2::pins(int theSCL, int theSDA)
{
	sda = theSDA;
	scl = theSCL;
...
uint8_t i2cEncoderLibV2::readEncoderByte(uint8_t reg)
{
	uint8_t rdata;
	i2c_start(bus);
	i2c_writeByte(bus, _add & writeMask); //sends i2c address w/ write bit set
	i2c_writeByte(bus, reg);
	i2c_start(bus);
	i2c_writeByte(bus, _add | readMask); //sends i2c address w/ read bit set
...
int32_t i2cEncoderLibV2::readEncoderLong(uint8_t reg)
{
	...
	/* use union to arrange bytes in correct order */
...
{
	_tem_data.val = data; //storing data in union allows writing in correct order

	i2c_start(bus);
	i2c_writeByte(bus, _add & writeMask); //sends i2c address w/ write bit set
	i2c_writeByte(bus, reg);			  //select register to write data
...
{
	_tem_data.fval = data; //storing data in union allows writing in correct order
